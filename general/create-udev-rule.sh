...
[ "$#" -eq 3 ] || { echo "Usage: $0 <BUS_NUM> <DEVICE_NUM> <DEVICE_NAME>"; exit 2; }
...
[ -z "$DEVICE_PATH" ] && { echo "Device $BUS_NUM:$DEVICE_NUM not found."; exit 3; }

SYSFS_PATH=$(find /sys/bus/usb/devices/usb*/ -name "*$DEVICE_PATH")
[ -z "$SYSFS_PATH" ] && { echo "Could not locate sysfs path for device $DEVICE_PATH."; exit 4; }
...
[ -e "/dev/$DEVICE_NAME" ] && { echo "/dev/$DEVICE_NAME successfully created!"; } || { echo "/dev/$DEVICE_NAME not yet created. Please check device and try again."; exit 5; }
